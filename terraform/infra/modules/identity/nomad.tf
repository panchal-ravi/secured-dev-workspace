...
# NOTE: the per-project, namespace-scoped Nomad ACL policy + binding rule
# (defense-in-depth, so a developer ever handed Nomad SSO is confined to their
# project namespace) is created PER PROJECT by the project-onboarding tier
# (modules/project), not here — this module only configures the global
# admin/readonly mapping.
