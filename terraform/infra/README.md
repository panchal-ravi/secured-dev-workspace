...
When the onboarding plane is enabled (`enable_platform_admin = true`), `developer-portal.tf` attaches the **`portal-blueprint-provisioning`** Vault policy (`portal-provisioning-policy.hcl`) to the portal's root WIF role `infra-developer-portal`. This is what lets a **project-admin** deploy a blueprint-backed MCP server into their project: the portal's blueprint Executor scopes every call to the project namespace via `client.WithNamespace(<project>)`, and the policy's **relative** paths then apply inside that namespace — mount/unmount secret engines, write the generated (lint-gated) ACL policies, manage the `jwt-nomad` WIF roles MCP jobs assume, configure database connections/roles, write class-B KV secrets, and revoke leases on deprovision.

> **Containment, read honestly.** Because the policy is attached in the **root** namespace with relative paths, it is usable in *any* namespace the portal targets — it is **not** physically pinned to one project by attachment scope. The boundary is application-level: the Executor always sends a project-namespace header (`DeployServer` rejects an empty `descriptor.Namespace`), every generated ACL policy is linted to the project's own mounts before it is written, and the policy's explicit `deny` stanzas forbid `identity/*`, `sys/auth*`, `sys/namespaces*`, and `cubbyhole/*`. A dedicated per-project provisioner identity (physical per-namespace pinning) is the documented future hardening — it needs portal→child-namespace token brokering that does not exist today. No project-tier change is required: the grant lives entirely on the portal's existing root identity.
