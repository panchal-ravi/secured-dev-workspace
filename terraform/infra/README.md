...
### Portal blueprint-provisioning grant (project-MCP deploy plane)

When the onboarding plane is enabled (`enable_platform_admin = true`), `developer-portal.tf` attaches the **`portal-blueprint-provisioning`** Vault policy (`portal-provisioning-policy.hcl`) to the portal's root WIF role `infra-developer-portal`. This is what lets a **project-admin** deploy a blueprint-backed MCP server into their project: the portal's blueprint Executor scopes every call to the project namespace via `client.WithNamespace(<project>)` — mount/unmount secret engines, write the generated (lint-gated) ACL policies, manage the `jwt-nomad` WIF roles MCP jobs assume, configure database connections/roles, write class-B KV secrets, and revoke leases on deprovision.

> **Cross-namespace ACL — the load-bearing detail (confirmed limitation).** A root-namespace token operating in a child namespace is ACL-matched against the **namespace-prefixed** path (`<child>/sys/policies/acl/…`), *not* the bare relative path — Vault resolves the namespace **before** path-matching, so a relative grant like `sys/policies/acl/*` matches nothing in a child, **and the `+/` single-segment wildcard does not match the namespace segment either**. Both **403**. Only an **explicit child prefix** (e.g. `acme/sys/policies/acl/*`) matches — proven while debugging the blueprint flow against live Vault Enterprise. Because project namespaces are **dynamic** (one per project, unknowable to this static infra-tier policy), the `+/`-prefixed `portal-provisioning-policy.hcl` **will 403 on a real deploy** as written. The project-deploy plane therefore needs the **§5 hardening** before instantiate/deprovision works in a project namespace: per-project policy attachment (the project tier appending a `<project>/…` grant to the portal role) or a dedicated per-namespace provisioner token brokered to the portal. Tracked separately. (Blueprint **validation** no longer touches Vault — it is shape + policy-lint only — so it is unaffected.)

> **Containment, read honestly.** The policy is attached in the **root** namespace, so it is usable in *any* namespace the portal targets — it is **not** physically pinned to one project by attachment scope. The boundary is application-level: the Executor always sends a project-namespace header (`DeployServer` rejects an empty `descriptor.Namespace`), every generated ACL policy is linted to the project's own mounts before it is written, and the policy's explicit `deny` stanzas forbid `identity/*`, `sys/auth*`, `sys/namespaces*`, and `cubbyhole/*`. No project-tier change is required for the grant itself: it lives entirely on the portal's existing root identity.
