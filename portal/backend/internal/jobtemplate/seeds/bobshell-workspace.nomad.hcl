# Project-owned Nomad job template for the IBM Bob Shell coding agent. Identical to
# dev-workspace.nomad.hcl EXCEPT it carries no Claude Code / LiteLLM wiring: Bob Shell
# has no LLM-provider/endpoint setting, so it cannot be pointed at the governed LiteLLM
# gateway. Its model runs on IBM's hosted backend (bob.ibm.com) and the developer signs
# in interactively with their own IBMid the first time they run `bob` in the workspace
# (per-user identity; the session token persists in ~/.bob on the durable /home/dev
# volume). MCP, git, SSH, and shared volumes work exactly as on the standard workspace.
#
...
#
# Project-static placeholders (filled at onboarding / project-template create):
...
# No llm_* placeholders — this template has no governed-LLM wiring (see header).
# MCP wiring is NOT in the base template — a project-admin attaches MCP servers from the
# catalog via structured template extension (injected at the @project-addons markers
# below); for this template the injection writes ~/.bob/mcp_settings.json.
#
...
job "${job_name}" {
  ...
  group "workspace" {
    ...
    # workspace's Boundary target at the replacement node.
    reschedule {
      unlimited      = true
      delay          = "15s"
...
# project-admin attaches MCP servers from the catalog. For the Bob Shell template
# each server is written into ~/.bob/mcp_settings.json (global scope) idempotently;
# a failure WARNs without aborting the workspace (a missing MCP tool must never cost
# the developer their SSH session). The base template registers none.
